...
4. 当错误被修复后，按下<esc>回到正常模式。重复步骤2到4以修改完整。
...
2. 在shell提示符下，键入此命令： vim tutor <enter>
'vim' 是要启动的vim编辑器，‘tutor'是编辑的文件名。该文件将被编辑修改。

3. Insert and delete text as you learned in the previous lessons.
3. 按照你在前几节你所学到的插入、删除文本。

4. Save the file with changes and exit Vim with: :wq <ENTER>
4. 保存对文件的修改，并退出vim: :wq <enter>

5. If you have quit vimtutor in step 1 restart the vimtutor and move down to
the following summary.
5. 如果你在步骤1中退出了本教程，启动本教程并移步到以的总结。
...
3. 用不同的计数重复步骤1和2，以一个命令删除多个连续的大写单词。
...
2. 输入 dd 以删除该行，该行会被存储至VIM寄存器中。

3. Move the cursor to the c) line, ABOVE where the deleted line should go.
3. 移动光标到c)行，即被删除行要到达的前行。

4. Type p to put the line below the cursor.
4. 键入 p 将那行粘贴到光标之下。

5. Repeat steps 2 through 4 to put all the lines in correct order.
5. 重复步骤2到4以粘贴所有的行到正确的位置。
...
5.键入 c$ 和 余下的同第二行相同的字等符，并按下<ESC>。
...
1. 要粘贴刚刚删除的文本，键入 p.这会把被删文本粘贴到光标之后（如果某行被删，它会被粘贴到光标之后的一行）。
...
3.更改操作码允许你更改从光标到对象定义处。例如：键入 ce 以更改光标到词尾，键入c$会更改光标到行尾。
...
2.现在，键入 'errroor'<ENTER>.这是你想要搜索的单词。
...
2.键入 :s/thee/the <ENTER>.注意此命令仅会更改本行的第一例"thee".
...
2. 现在键入 ! 字符。这允许你执行一个任何外部的shell命令

3. As an example type ls following the ! and then hit <ENTER>. This
will show you a listing of your directory, just as if you were at the
shell prompt. Or use :!dir if ls doesn't work.
3. 作为一个例子，键入 ls 接一个!,然后敲击<enter>.这会显示一个你的目录，
...
1. 键入 :!dir or :!ls 以得到你的目录的清单。
你已经 知道了你必须在这句后 敲击<enter>.
2. Choose a filename that does not exist yet, such as TEST.
2. 选择一个不存在的文件名，如TEST。

3. Now type: :w TEST (where TEST is the filename you chose.)
3. 现在 键入 : :w TEST (其中的TEST是你选择的文件名）。

4. This saves the whole file (the Vim Tutor) under the name TEST.
To verify this, type :!dir or :!ls again to see your directory.
4. 这会以TEST的名字 保存全部文件（即这个 VIM教程）。
...
text is highlighted.
2. 按 v 并移动 光标到 以下的第5项。 注意这些文本会珵高亮。

3. Press the : character. At the bottom of the screen :'<,'> will appear.
3. 按 ：字符。在屏幕底会显示 :'<,'> will appear.

4. Type w TEST , where TEST is a filename that does not exist yet. Verify
that you see :'<,'>w TEST before you press Enter.
4. 按 w TEST ,其中 TEST 是一个不存在 的文件。
确认你按<enter>前你看到了:'<,'>w TEST.

5. Vim will write the selected lines to the file TEST. Use :!dir or !ls
to see it. Do not remove it yet! We will use it in the next lesson.
5. vim 会把选定的行进文件TEST中,使用 :!dir or !ls 作检查。先别删除它！
我们会下节使用它。
...
2. 现在 恢复 你的TEST文件，使用命令 ：r TEST 其中，TEST 是你使用的文件。
你要恢复的文件将被放置在光标行下面。

3. To verify that a file was retrieved, cursor back and notice that there
are now two copies of Lesson 5.3, the original and the file version.
3. 要确认文件被恢复了，用光标返回，注意这里有两5.3节的副本，原本的文件版。
...
2. :w FILENAME 将当前的vim 文件以FILENAME 文件名写进磁盘 。

3. v motion :w FILENAME saves the Visually selected lines in file
FILENAME.
3. v 对象 :w FILENAME 保存选定选择行到FILENAME 文件。

4. :r FILENAME retrieves disk file FILENAME and puts it below the
cursor position.
4. :r FILENAME 恢复磁盘文件FILENAME到光标下。

5. :r !dir reads the output of the dir command and puts it below the
cursor position.
5. :r !dir 读取dir命令的输出并粘贴到光标下。
...
you in Insert mode.
2. 键入 小写字母 o 以在光标下插入一行，并置于插入模式。

3. Now type some text and press <ESC> to exit Insert mode.
3. 现在 键入 一些文本，并按<esc>以退出插入模式。 

---> After typing o the cursor is placed on the open line in Insert mode.

4. To open up a line ABOVE the cursor, simply type a capital O , rather
than a lowercase o. Try this on the line below.
4. 要在光标前插入一行，只需键入 大写字母 O，而非小写字母 o，在下一行试试。
...
3. 铵 一个 a(小写） 以追加文本到光标后。

4. Complete the word like the line below it. Press <ESC> to exit Insert
mode.
4. 按照下一行完成这个单词。按<esc>退出插入模式。

5. Use e to move to the next incomplete word and repeat steps 3 and 4.
5. 使用 e 移动到未完成的单词，并重复步骤3和4.
...
1. 移动光标到以---》标记的第一行。移动光标到第一个XXX。

2. Now press R and type the number below it in the second line, so that it
replaces the xxx .
2. 现在 按下 R 并键入 在第二行显示的数字，以其替换xxx.

3. Press <ESC> to leave Replace mode. Notice that the rest of the line
remains unmodified.
3. 按下<ESC> 以 离开替换模式。注意 剩余的行仍没有改变。
...
5. 按p 以粘贴文本，然后键入：a second <esc>.

6. Use Visual mode to select " item.", yank it with y , move to the end of
the next line with j$ and put the text there with p .
6. 使用选择模式以选择” item。“，以y复制之，以j$移动到下行并以p粘贴到那里。
...
1. 搜索’ignore‘ ，键入 ： /ignore <enter>
重复按n几次。

2. Set the 'ic' (Ignore case) option by entering: :set ic
2. 设置 ’ic‘（忽略大小写）选项，键入： ：set ic
...
4. 设置选项'hlsearch' and 'incsearch': set hls is

5. Now type the search command again and see what happens: /ignore <ENTER>
5. 同在键入搜索命令，看看发生 了什么： /ignore <enter>

6. To disable ignoring case enter: :set noic
6. 要禁止忽略大小写，键入： :set noic 
...
1. 确保VIM 处于非兼容模式： :set nocp 

2. Look what files exist in the directory: :!ls or :!dir
2. 看看目录下有什么文件存在： :!ls or :!dir 

3. Type the start of a command: :e
3. 键入命令的开始部分： :e 

4. Press CTRL-D and Vim will show a list of commands that start with "e".
4. 按 CTRL-D ，VIM会显示以“e”开头的清单。

5. Press <TAB> and Vim will complete the command name to ":edit".
5. 按下<TAB>，VIM会以“：edit"补齐命令名。

6. Now add a space and the start of an existing file name: :edit FIL
6. 现在 添加一个空格和己存的文件名头部： :edit FIL

7. Press <TAB>. Vim will complete the name (if it is unique).
7. 按<tab>，vim会完成余下的文件名（如果它是独有的话）。
...
1. 键入 ：help 或 <F1> 或<help>以打开帮助窗口

2. Type :help cmd to find help on cmd .
2. 键入 ：help cmd 以找出关于cmd的帮助 。

3. Type CTRL-W CTRL-W to jump to another window
3. 键入 CTRL-W 以跳转到另一窗口。
...
6. 当键入 ：command 时，按下CTRL-D 以查看可能的补齐。
按<tab>使用其中一个补齐。
